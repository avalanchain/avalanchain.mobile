...
namespace avalanchain
{
    public static class SampleData
    {
        ...
        private static List<CryptocurrencyWallet> _wallets;
        ...
        public static User CurrentUser => _currentUser ?? (_currentUser = InitCurrentUser());
        ...
        public static List<Account> Accounts => InitAccounts();
        ...
        public static List<CryptocurrencyWallet> Wallets
        {
            get
            {
                if (_wallets == null)
        ...
        public static Task<CurrencyPricing> CryptocurrencyPrices
        {
            get
            {
        ...
        public static List<Transaction> Transactions
        {
            get
            {
                if (_trnasactions == null)
                {
                    _trnasactions = InitTrnasactions();
                }

                return _trnasactions;
            }
        }
    ...
    //                "June 7",
        ...
        private static List<Card> InitCards()
        {
            return new List<Card> {
                ...
                new Card {
                    Id              = Guid.NewGuid(),
                    AccountNumber   = "4422556677889901",
                    Name            = "Pound Card",
                    Currency        = CurrencyType.GBP,
                    CurrencyIcon    = FontAwesome.FAGbp,
                    TypeIcon         = FontAwesome.FAFaVisa,
                    Amount          = 89.90m
                },

                new Card {
                    Id              = Guid.NewGuid(),
                    AccountNumber   = "4422556677889902",
                    Name            = "USD Card",
                ...
                new Card {
                    Id              = Guid.NewGuid(),
                    AccountNumber   = "4422556677889903",
                    Name            = "EUR",
                    Currency        = CurrencyType.EUR,
                    CurrencyIcon            = FontAwesome.FAEur,
                    TypeIcon         = FontAwesome.FAFaMasterCard,
                    Amount           = 2089.90m
                },

                new Card {
                    Id              = Guid.NewGuid(),
                    AccountNumber   = "4422556677889904",
                    Name            = "EUR",
                    Currency        = CurrencyType.EUR,
                    CurrencyIcon            = FontAwesome.FAEur,
                    TypeIcon         = FontAwesome.FAFaVisa,
        ...
        private static List<Transaction> InitTrnasactions()
        {
            ...
            foreach (Account acc in accnts)
            {
                ...
                for (var i = 0; i < countTransactionsOfAcc; i++)
                {
                    var icon = FontAwesome.FAAngleDoubleRight;
                    TransactionType transac;
                    Random rnd = new Random();
                    int ran = rnd.Next(1, 30);

                    var amount = acc.Currency == CurrencyType.BTC ? 0.0001 * ran * i + 0.00012 : ran * i + 21;
                    var percent = amount / 100;
                    var fee = acc.Currency == CurrencyType.BTC ? (decimal)Math.Round(percent, 6) : (decimal)Math.Round(percent, 2);
                    var dateTime = DateTime.UtcNow;



                    if (i == 0) { transac = TransactionType.Pay; icon = FontAwesome.FAShoppingCart; }
                    else if (i % 2 == 0) { transac = TransactionType.Sent; icon = FontAwesome.FAAngleDoubleRight; dateTime = dateTime.AddDays(1); }
                    else { transac = transac = TransactionType.Receive; icon = FontAwesome.FAAngleDoubleLeft; dateTime = dateTime.AddDays(2); }
                    ...
                    transactions.Add(new Transaction
                    {
                        ...
                        CurrencyIcon = acc.CurrencyIcon,
                        TypeIcon = icon,
                        Type = transac,
                        Amount = (decimal)amount,
                        DateTime = dateTime,
                        Fee = fee,
                        AmountReceive = (decimal)amount,
                    });
        ...
        private static async Task<CurrencyPricing> InitCryptocurrencyPrices()
        {

            var cryptocurrencyPrices = new CurrencyPricing();
            //var nativeCurrency = CurrentUser.NativeCurrency;
            //var currs = "BTC,ETH,EUR,GBP,USD";
            //currs = currs.Replace(CurrentUser.NativeCurrency.ToString(), string.Empty);
            //var link = "https://min-api.cryptocompare.com/data/price?"+ string.Format("fsym={0}&tsyms=", nativeCurrency)+ "BTC,ETH,EUR,GBP";
